...
(function ($, window, document, undefined) {
  ...
  var Tree = function Tree(element, options) {
    ...
    this.styleId = "".concat(this.elementId, "-style");
    this.init(options);
    return {
  ...
  Tree.prototype.buildStyleOverride = function (node) {
    ...
    return "color:".concat(color, ";background-color:").concat(backColor, ";");
  }; // Add inline style into head


  Tree.prototype.injectStyle = function () {
    if (this.options.injectStyle && !document.getElementById(this.styleId)) {
      $("<style type=\"text/css\" id=\"".concat(this.styleId, "\"> ").concat(this.buildStyle(), " </style>")).appendTo('head');
    }
  }; // Construct trees style based on user options


  Tree.prototype.buildStyle = function () {
    var style = ".node-".concat(this.elementId, "{");

    if (this.options.color) {
      style += "color:".concat(this.options.color, ";");
    }

    if (this.options.backColor) {
      style += "background-color:".concat(this.options.backColor, ";");
    }

    if (!this.options.showBorder) {
      style += 'border:none;';
    } else if (this.options.borderColor) {
      style += "border:1px solid ".concat(this.options.borderColor, ";");
    }

    style += '}';

    if (this.options.onhoverColor) {
      style += ".node-".concat(this.elementId, ":not(.node-disabled):hover{") + "background-color:".concat(this.options.onhoverColor, ";") + '}';
    }
  ...
  function treeview(options, args) {
    var result;
    this.each(function () {
      ...
      if (typeof options === 'string') {
        if (!_this) {
          logError("Not initialized, can not call method : ".concat(options));
        } else if (!$.isFunction(_this[options]) || options.charAt(0) === '_') {
          logError("No such method : ".concat(options));
        } else {
...
})(_jquery.default, window, document);

var _default2 = _jquery.default.fn.treeview;
