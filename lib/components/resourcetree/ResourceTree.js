...
function (_TagsComponent) {
  ...
  _createClass(ResourceTreeComponent, [{
    ...
    value: function addInput(input, container) {
      ...
      _get(_getPrototypeOf(ResourceTreeComponent.prototype), "addInput", this).call(this, input, table.querySelector('#tags')); // super.disabled = true;


      var jqinit = (0, _jquery.default)('#tree');

      if (jqinit.treeview === undefined) {
        jqinit.treeview = _bootstrapTreeview.default;
      }

      this.getTree().then(function (res, err) {
        if (err) {
        ...
        (0, _jquery.default)('#tree').treeview({
          ...
          onNodeChecked: function onNodeChecked(event, data) {
            _this2.nodeChecked(data);
          },
          onNodeUnchecked: function onNodeUnchecked(event, data) {
            _this2.nodeUnchecked(data);
          }
        });

        if (!_this2.treeRedrawn) {
          _this2.treeRedrawn = true;

          _this2.triggerRedraw();
        }
      });
    }
  }, {
    ...
    value: function nodeChecked(node) {
      var newVal = {
        value: node,
        label: _lodash.default.get(node, this.component.titleAttribute),
        id: _lodash.default.get(node, this.component.idAttribute)
      };

      _get(_getPrototypeOf(ResourceTreeComponent.prototype), "setValue", this).call(this, _toConsumableArray(_get(_getPrototypeOf(ResourceTreeComponent.prototype), "getValue", this).call(this)).concat([newVal]));

      this.data[this.component.key] = _toConsumableArray(this.data[this.component.key]).concat([node]);
      this.choices.render();
    }
  }, {
    key: "nodeUnchecked",
      ...
      response.map(function (val) {
        if (Array.isArray(val)) {
          val.forEach(function (v) {
            _this3.prepareTreeAttributes(v);
          });
        ...
        val.text = _lodash.default.get(val, _this3.component.titleAttribute);

        var children = _lodash.default.get(val, _this3.component.childrenAttribute);

        if (children) {
          _this3.prepareTreeAttributes(children);
    ...
    value: function getTree() {
      ...
      if (this.component.url && this.component.url.trim() !== '') {
        return _Formio.default.makeStaticRequest(this.component.url, 'GET').then(function (res, err) {
          ...
          return _this4.handleTreeResponse(res);
        });
      } else {
        var fakeData = {
