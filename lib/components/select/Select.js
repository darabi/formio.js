...
function (_BaseComponent) {
  ...
  function SelectComponent(component, options, data) {
    ...
    _this2 = _possibleConstructorReturn(this, _getPrototypeOf(SelectComponent).call(this, component, options, data)); // Trigger an update.

    _this2.triggerUpdate = _lodash.default.debounce(_this2.updateItems.bind(_assertThisInitialized(_this2)), 100); // Keep track of the select options.

    _this2.selectOptions = []; // Keep track of the last batch of items loaded.

    _this2.currentItems = [];
    _this2.loadedItems = 0;
    _this2.isScrollLoading = false;
    _this2.scrollTop = 0; // If this component has been activated.

    _this2.activated = false; // Determine when the items have been loaded.

    _this2.itemsLoaded = new Promise(function (resolve) {
  ...
  _createClass(SelectComponent, [{
    ...
    value: function addAutofillHoneyInput(container, input) {
      ...
      autofillInput.addEventListener('change', function (event) {
        _this3.updateValue({}, JSON.parse(event.target.value));
      });
    ...
    value: function loadItems(url, search, headers, options, method, body) {
      ...
      if (this.component.searchField && search) {
        if (Array.isArray(search)) {
          ...
          query["".concat(this.component.searchField, "__regex")] = "/".concat(search, "/i");
        }
      ...
      _Formio.default.makeRequest(this.options.formio, 'select', url, method, body, options).then(function (response) {
        ...
        var scrollTop = !_this5.scrollLoading && _this5.currentItems.length === 0;

        _this5.setItems(response, !!search);

        if (scrollTop && _this5.choices) {
          _this5.choices.choiceList.scrollToTop();
        }
      }).catch(function (err) {
        _this5.stopInfiniteScroll();

        _this5.loading = false;

        _this5.itemsLoadedResolve();

        _this5.emit('componentError', {
          component: _this5.component,
          message: err.toString()
        });

        console.warn("Unable to load resources for ".concat(_this5.key));
      });
    }
    /**
    ...
    value: function addInput(input, container) {
      ...
      var searchField = this.component.searchField; // The following function gets 'this' from Choices ...

      var _this = this;

      var callbackOnUnknownChoice = function callbackOnUnknownChoice(value, hasShift) {
        var _this7 = this;
        ...
        if (this._store.activeItems.findIndex(function (x) {
          return x.value.data.text === value;
        }) > -1) {
          alert("Tag \"".concat(value, "\" wurde bereits ausgew\xE4hlt!"));
          return reject();
        }

        var idx = this._store.choices.findIndex(function (x) {
          return x.value.data.text === value;
        });

        if (idx > -1) {
          var item = this._store.choices[idx];
          console.log("Tag \"".concat(value, "\" existiert bereits: "), item);
        ...
        if (!hasShift) {
          if (!confirm("Neuer Tag \"".concat(value, "\" ?"))) {
            return reject();
          }
        }

        var dialog = _this.createModal(_this.component.addResourceLabel || 'Add Resource');
        ...
        form.on('submit', function (submission) {
          console.log("Created new tag \"".concat(value, "\""));
          var newTagObj = submission;

          if (_this.component.multiple) {
            submission = [].concat(_toConsumableArray(_this.dataValue), [submission]);
          }
          ...
          _this7.setChoices([newItem], 'value', 'label', false);

          var idx = _this7._store.choices.findIndex(function (x) {
            return x.value.data.text === value;
          });

          if (idx > -1) {
            var _item = _this7._store.choices[idx]; //console.log('Activating item ',item.id);

            _this7.highlightItem(_item);

            var _activeItems = _this7._store.activeItems;

            if (_activeItems[0]) {
              _activeItems[0].keyCode = 13;
            }

            _this7._handleChoiceAction(_this7._store.activeItems, null, _item.id);
          } else {
        ...
        form.src = "".concat(_lodash.default.get(this.root, 'formio.projectUrl', _Formio.default.getBaseUrl()), "/form/").concat(_this.component.resource);
        return true;
      };
      ...
      var choicesOptions = _objectSpread({
        ...
        itemComparer: function itemComparer(a, b) {
          if (_typeof(a) == 'object' && _typeof(b) == 'object' && a._id && b._id) {
            return a._id === b._id;
          } else {
      ...
      this.onScroll = function () {
        if (!_this6.scrollLoading && _this6.scrollList.scrollTop + _this6.scrollList.clientHeight >= _this6.scrollList.scrollHeight) {
          _this6.scrollTop = _this6.scrollList.scrollTop;
          _this6.scrollLoading = true;

          _this6.triggerUpdate(_this6.choices.input.element.value);
        }
      };
      ...
      if (placeholderValue && this.choices._isSelectOneElement) {
        this.addEventListener(input, 'removeItem', function () {
          var items = _this6.choices._store.activeItems;

          if (!items.length) {
            _this6.choices._addItem(placeholderValue, placeholderValue, 0, -1, null, true, null);
          }
        });
      } // Add value options.
