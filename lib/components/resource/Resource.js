  ...
  _createClass(ResourceComponent, [{
      ...
      this.addEventListener(addButton, 'click', function (event) {
        ...
        form.on('submit', function (submission) {
          if (_this2.component.multiple) {
            submission = _toConsumableArray(_this2.dataValue).concat([submission]);
          }
    ...
    value: function addInput(input, container) {
      ...
      if (this.component.addResource) {
        ...
        var template = '<tbody>' + '<tr>' + '<td id="select" class="resource-select">' + '</td>' + '</tr>' + '<tr>' + '<td id="button" class="add-resource-button" colspan="2">' + '</td>' + '</tr>' + '</tbody>';
        container.appendChild(table);
        table.innerHTML = template;
        table.querySelector('.add-resource-button').appendChild(this.addButton());

        _get(_getPrototypeOf(ResourceComponent.prototype), "addInput", this).call(this, input, table.querySelector('.resource-select'));
      } else {
